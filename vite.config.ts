...
function routeFallbackHtml(route: StaticRoute) {
  const qualityText =
    ...
    `<h2>차량 유형별 검토 기준</h2>` +
    `<p>경차와 소형차는 도심 주행, 주차 편의성, 보험료 부담을 줄이는 데 유리하지만 장거리 이동과 가족 탑승이 많다면 공간과 승차감을 다시 봐야 합니다. SUV와 미니밴은 적재 공간과 활용도가 높지만 타이어, 연료비, 자동차세, 주차 비용이 커질 수 있습니다. 수입차는 보증 종료 후 정비비, 부품 수급, 보험료 상승 가능성까지 고려해야 합니다.</p>` +
    `<h2>계약 단계 체크포인트</h2>` +
    `<p>계약 전에는 차량 가격, 선택 옵션, 등록 비용, 탁송료, 할인 조건, 금융 수수료를 분리해 확인하세요. 장기렌트와 리스는 보험 포함 여부, 정비 포함 범위, 약정 주행거리, 중도 해지 위약금, 만기 인수 가격이 총비용을 바꿉니다. 중고차는 성능점검기록부, 보험 이력, 소유자 변경 이력, 침수와 사고 여부, 타이어와 브레이크 상태를 함께 확인해야 합니다.</p>` +
    `<h2>최신 비용 정보 확인</h2>` +
    `<p>자동차세, 전기차 보조금, 보험 특약, 환경 규제, 유류세와 충전 요금은 시점과 지역에 따라 달라질 수 있습니다. 카테인은 이런 정보를 확정된 개인 견적처럼 표현하지 않고 확인해야 할 기준으로 안내합니다. 글에서 비용 구조를 이해한 뒤 공식 고시, 제조사 안내, 보험사 계산 결과를 함께 대조하면 광고성 추천이나 단편적인 후기보다 더 안정적인 결정을 할 수 있습니다.</p>` +
    `<h2>카테인을 반복해서 활용하는 방법</h2>` +
    `<p>자동차 구매는 한 번의 검색으로 끝나지 않는 경우가 많습니다. 처음에는 예산과 차량 유형을 정하고, 다음에는 유지비 계산기로 월 부담을 비교한 뒤, 마지막에는 매거진 글로 보험, 세금, 정비, 계약 조건을 점검하는 흐름이 효율적입니다. 후보 차량이 바뀌면 같은 조건으로 다시 계산해 보고, 가족 구성원이나 운행 목적이 달라질 때도 비용 항목을 새로 확인해야 합니다.</p>` +
    `<p>사이트의 글은 실사용자가 이해하기 쉬운 언어로 작성하되, 비용 판단에 필요한 항목을 빠뜨리지 않는 것을 목표로 합니다. 특정 브랜드나 판매 조건을 그대로 믿기보다 여러 자료를 비교하고, 실제 계약서와 보험 산출 결과를 확인한 뒤 결정하는 습관이 장기적인 자동차 비용을 줄이는 데 도움이 됩니다.</p>` +
    `<p>운전자의 상황이 바뀌면 적합한 차량도 달라집니다. 이사, 출퇴근 거리 변화, 가족 구성 변화, 주행거리 증가, 보험 조건 변경이 생겼다면 기존 계산 결과를 그대로 쓰지 말고 최신 조건으로 다시 비교하세요.</p>` +
