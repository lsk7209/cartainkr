...
export const useSEO = ({
  ...
  useEffect(() => {
    ...
    // OG/Twitter tags are read by Google and social platforms — use full length (60/160 char guidance).
    const naverTitle = title.length > 40 ? title.slice(0, 40) : title;
    const naverDesc =
      description.length > 80 ? description.slice(0, 80) : description;
    const ogDesc =
      description.length > 160 ? description.slice(0, 160) : description;

    // Update title (Naver reads <title>)
    document.title = naverTitle;
    ...
    // Basic meta tags — Naver limits (40/80자)
    setMeta("description", naverDesc);
    if (keywords?.length) {
    ...
    // Twitter Card tags — same length budget as OG (Google/social)
...
export const generateWebSiteSchema = () => {
  ...
  return {
    ...
    url: baseUrl,
    description:
      "자동차 구매, 유지비, 세금 계산 등 스마트한 자동차 정보를 제공합니다.",
    inLanguage: "ko-KR",
    potentialAction: {
      "@type": "SearchAction",
      target: {
...
export const generateHowToSchema = (url: string) => {
  return {
    ...
    description:
      "차량 가격, 할부 조건, 연비, 보험료를 입력하면 월 유지비를 자동으로 계산해드립니다.",
    url,
    totalTime: "PT2M",
